...
function stepThrough(node, debug=false, firstTime=true) {
  ...
  while (nodeStatus.hasChanged()) {
    steps = addStep(steps, nodeStatus, debug);
    nodeStatus.newNode = clone(nodeStatus.newNode);
    nodeStatus.oldNode = null;
    // TODO: use this instead when we can use node.clone()
...
function addStep(steps, nodeStatus, debug) {
  ...
  let oldLatex = prettyPrint(nodeStatus.oldNode, true, false, true);
  let newLatex = prettyPrint(nodeStatus.newNode, true, false, true);
