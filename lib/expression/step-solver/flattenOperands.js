...
function flattenOperands(node) {
  if (NodeType.isConstant(node, true)) {
    ...
    const constNode = NodeCreator.constant(node.eval());
    if (node.changeGroup) {
    ...
    const changeGroup = node.changeGroup; // TODO: I don't like this. I want some way to preserve change groups better. I should probably write my own clone() and
    let arg = flattenOperands(node.args[0]);
    const newNode = Negative.negate(arg, true);
    if (changeGroup) {
      newNode.changeGroup = changeGroup;
    }
    return newNode;
  }
...
function getOperands(node, parentOp) {
  ...
  if (PolynomialTermNode.isPolynomialTerm(node, false, true)) {
...
function maybeFlattenPolynomialTerm(node) {
  ...
  if (NodeType.isConstantOrConstantFraction(lastOperand)) {
